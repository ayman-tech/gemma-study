...
# HPC     : compatible with SLURM — see job_int4.sh
#
# Usage:
#   python train_int4_hpc.py                        # full dataset, 3 epochs
#   python train_int4_hpc.py --num_rows 500         # 500 rows
#   python train_int4_hpc.py --epochs 5             # 5 epochs
#   python train_int4_hpc.py --gpu                  # force GPU (error if unavailable)
#   python train_int4_hpc.py --no_gpu               # force CPU
# =============================================================================
...
# 1. ARGUMENT PARSING
# =============================================================================
parser = argparse.ArgumentParser(description="Fine-tune Gemma-3 with QLoRA INT4 on HPC")
...
# -----------------------------------------------------------------------------
OUTPUT_DIR   = "/home/ziyadh10/scratch/gemma_project/output_int4"
...
if HAS_GPU:
    ...
    print("   No GPU — using CPU (INT4 may fall back to float32)")
    DEVICE_MAP    = "cpu"
    NO_CUDA       = True
...
# =============================================================================
# 7. BUILD INT4 CONFIG & LOAD MODEL
# =============================================================================
print("[3/6] Configuring INT4 quantization and loading model ...")
...
# -----------------------------------------------------------------------------
# HPC FIX 5: Always call prepare_model_for_kbit_training for INT4
# (original script only called this on GPU — should always run for quantized models)
# -----------------------------------------------------------------------------
...
sft_config = SFTConfig(
    ...
    bf16=use_bf16,
    # -------------------------------------------------------------------------
    # HPC FIX 6: gradient_checkpointing saves significant VRAM on HPC
    # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # HPC FIX 7: adamw_torch instead of paged_adamw_8bit
    # paged_adamw_8bit can break on PyTorch 2.0.1 + certain bitsandbytes versions
    # -------------------------------------------------------------------------
    optim="adamw_torch",
    no_cuda=NO_CUDA,
...
print("\n" + "=" * 60)
print("  TRAINING METRICS SUMMARY  —  INT4")
print("=" * 60)
